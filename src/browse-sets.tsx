...
export default function BrowseSets() {
  ...
  useEffect(() => {
    console.log(
      `[BrowseSets] effect fired — allSets.length: ${allSets.length}, setFilter: ${setFilter}, fetchTrigger: ${fetchTrigger}, storageLoaded: ${storageLoaded}`
    );
    ...
    console.log(
      `[BrowseSets] calling startCoverFetch — filter: ${setFilter}, codes: ${codes.length}, cached: ${cached.size}`
    );

    startCoverFetch(codes, cached);

    return () => console.log(`[BrowseSets] effect cleanup — filter: ${setFilter}`);
    // Intentionally narrow deps: re-run only when the set list, filter, manual
    // trigger, or storage-loaded state changes — not on every persistedCards update.
