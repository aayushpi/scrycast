...
function CardDetailView({ card }: { card: Card }) {
  ...
  // Markdown: image only — fills the left content area
  // Metadata: all text fields on the right panel
  // (HTML tables don't render in Raycast — only <img> tags are supported)
  const markdown = `![${card.name}](${imageUri})`;

  // Oracle text: split each ability onto its own Label so lines wrap instead of truncating
